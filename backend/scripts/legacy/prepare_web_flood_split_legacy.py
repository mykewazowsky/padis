...
for rp, cols in scenarios.items():
    for climate_key, loss_col in cols.items():
        print(f"Menyiapkan flood {climate_key} {rp} ...")
        ...
        output_path = os.path.join(OUTPUT_DIR, f"web_flood_{climate_key}_{rp}.geojson")
        out.to_file(output_path, driver="GeoJSON", engine="fiona")

        size_mb = os.path.getsize(output_path) / (1024 * 1024)
        print(f"{output_path} -> {size_mb:.2f} MB")

print("Selesai: web flood split layers siap.")
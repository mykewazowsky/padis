...
def add_aal_to_layer(layer_path, aal_csv_path, aal_nonclimate_col, aal_climate_col):
    ...
    # hapus kolom lama kalau sudah ada, biar aman saat rerun
...
def process_multi():
    ...
    for scenario in SCENARIOS:
        for climate in CLIMATES:
            layer_path = os.path.join(OUTPUT_DIR, f"web_multi_{climate}_{scenario}.geojson")
            add_aal_to_layer(
                layer_path,
                aal_csv,
                "aal_nonclimate",
...
def process_flood():
    ...
    for scenario in SCENARIOS:
        for climate in CLIMATES:
            layer_path = os.path.join(OUTPUT_DIR, f"web_flood_{climate}_{scenario}.geojson")
            add_aal_to_layer(
                layer_path,
                aal_csv,
...
def process_drought():
    ...
    for scenario in SCENARIOS:
        for climate in CLIMATES:
            layer_path = os.path.join(OUTPUT_DIR, f"web_drought_{climate}_{scenario}.geojson")
            add_aal_to_layer(
                layer_path,
                aal_csv,
...
if __name__ == "__main__":
    ...
    process_flood()
    process_drought()
    print("\nSelesai: AAL berhasil ditambahkan ke semua split web layer.")
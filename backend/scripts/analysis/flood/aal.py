...
def compute_aal_flood(df: pd.DataFrame) -> pd.DataFrame:

    # =========================
    # DETECT COLUMNS
    # =========================
    nonclimate_cols = [c for c in df.columns if c.startswith("loss_flood_nonclimate_")]
    climate_cols = [c for c in df.columns if c.startswith("loss_flood_climate_")]

    if not nonclimate_cols:
        raise ValueError("Kolom loss_flood_nonclimate_* tidak ditemukan")
    if not climate_cols:
        raise ValueError("Kolom loss_flood_climate_* tidak ditemukan")

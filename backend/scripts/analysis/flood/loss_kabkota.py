...
def compute_loss_flood_kab(gdf, prod_df, gabah):
    ...
    # DETECT LOP COLUMNS
    # =========================
    lop_cols = [c for c in gdf.columns if c.startswith("lop_flood_")]

    if not lop_cols:
        raise ValueError("Tidak ada kolom lop_flood_* ditemukan")
    ...
    for col in lop_cols:
        ...
        if "rc" in col:
            out_col = f"loss_flood_climate_rp{rp}"
        else:
            out_col = f"loss_flood_nonclimate_rp{rp}"
    ...
    # =========================
    # CLEAN GEOMETRY (optional but recommended)
    # =========================
    gdf_kab = gdf_kab.set_geometry("geometry")

    # =========================
    # OPTIONAL: ROUND (BIAR FILE KECIL)
    # =========================

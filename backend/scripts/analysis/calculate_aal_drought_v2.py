...
output_csv = os.path.join(OUTPUT_DIR, "kabkota_drought_aal_v2.csv")
...
require_columns(
    ...
    "kabkota_drought_loss_std"
)

print(f"Menghitung AAL drought non-iklim... (tail_method={TAIL_METHOD})")
df["aal_drought_nonclimate_v2"] = df.apply(
    lambda row: calculate_aal(row, nonclimate_cols, tail_method=TAIL_METHOD),
    axis=1
)

print(f"Menghitung AAL drought iklim... (tail_method={TAIL_METHOD})")
df["aal_drought_climate_v2"] = df.apply(
    lambda row: calculate_aal(row, climate_cols, tail_method=TAIL_METHOD),
    axis=1
)

matched_nonclimate = df["aal_drought_nonclimate_v2"].notna().sum()
matched_climate = df["aal_drought_climate_v2"].notna().sum()

print(f"\nAAL drought non-iklim terhitung: {matched_nonclimate}/{len(df)}")
print(f"AAL drought iklim terhitung: {matched_climate}/{len(df)}")

print("\nStatistik AAL drought non-iklim v2:")
print(df["aal_drought_nonclimate_v2"].describe())

print("\nNULL AAL drought non-iklim v2:")
print(df["aal_drought_nonclimate_v2"].isnull().sum())

print("\nStatistik AAL drought iklim v2:")
print(df["aal_drought_climate_v2"].describe())

print("\nNULL AAL drought iklim v2:")
print(df["aal_drought_climate_v2"].isnull().sum())
...
print("Menyimpan hasil AAL drought v2 ke CSV...")
df_out.to_csv(output_csv, index=False)

print(f"Selesai: AAL drought v2 berhasil dihitung -> {output_csv}")
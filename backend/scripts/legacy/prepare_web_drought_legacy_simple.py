...
scenarios = {
    "rp25": "loss_drought_nonclimate_rp25",
    "rp50": "loss_drought_nonclimate_rp50",
    "rp100": "loss_drought_nonclimate_rp100",
    "rp250": "loss_drought_nonclimate_rp250",
}
...
for key, loss_col in scenarios.items():
    print(f"Menyiapkan web drought layer {key} ...")
    ...
    output_path = os.path.join(OUTPUT_DIR, f"web_drought_{key}.geojson")
    out.to_file(output_path, driver="GeoJSON", engine="fiona")

    size_mb = os.path.getsize(output_path) / (1024 * 1024)
    print(f"{output_path} -> {size_mb:.2f} MB")

print("Selesai: web drought layers siap.")